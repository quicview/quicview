...
#[tokio::test]
async fn quic_ctrl_reauth_flow() {
    ...
    // Ask server to trigger reauth
    tx_sig.send(quic_ctrl::ServerSignal::ReauthRequest).unwrap();

    // Wait for AuthRenewed event
    let mut got = false;
    for _ in 0..10 {
        match events.recv().await.expect("event2") {
            ClientEvent::CtrlAuthRenewed => { got = true; break; }
            _ => continue,
        }
    }
    assert!(got, "expected CtrlAuthRenewed after reauth request");
}
