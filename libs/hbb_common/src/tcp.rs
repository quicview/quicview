//! TCP stream utilities with framing and encryption support.
//!
//! NOTE: For new QuicView code, prefer using `crates/transport` for QUIC/TCP connections.
...
pub async fn listen_any(port: u16) -> ResultType<TcpListener> {
    // Optional: force loopback bind for TCP listeners when terminating TLS externally.
    if matches!(std::env::var("TCP_LISTEN_LOOPBACK").ok().as_deref(), Some("1"|"Y"|"true"|"True"|"TRUE")) {
        // Try IPv6 loopback first, then IPv4.
        if let Ok(l) = new_socket(
